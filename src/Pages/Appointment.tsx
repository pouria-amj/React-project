...
const Appointment:FC = ()=>{
    ...
    const showCalendar = () => {
       ...
       for (let p = 0; p < fDay; p++) {
          allDays.push(<div key = {`em-${p}`} className = "bg-[#e0e0e0] cursor-default"></div>);
       }
       ...
       for (let d = 1; d <= mDays; d++) {
          ...
          allDays.push(
             <div
                key = {`d-${d}`}
                className = {`h-8 bg-[#f0f0f0] border border-solid border-[#ccc] flex justify-center items-center cursor-pointer ${isSelected ? "bg-blue-500 " : ""}`}
...
<div className="flex justify-center">
      ...
      <div className = "w-[320px] p-[0.7rem]  border border-solid border-blue-100 rounded-md bg-slate-50">
         <div className = "flex mb-[11.2px] justify-between items-center">
            <button className="cursor-pointer text-2xl" onClick = {changeToPrevMonth}> {"<"} </button>
            <h2>
            ...
            <button className="cursor-pointer text-2xl" onClick = {changeToNextMonth}> {">"} </button>
         </div>
